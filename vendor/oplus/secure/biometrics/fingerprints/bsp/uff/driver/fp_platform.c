...
int fp_parse_dts(struct fp_dev *fp_dev) {
    ...
    fp_dev->reset_gpio = of_get_named_gpio(np, "goodix,gpio_reset", 0);
    if (fp_dev->reset_gpio < 0) {
        pr_err("falied to get reset gpio!\n");
        return fp_dev->reset_gpio;
    }

    rc = devm_gpio_request(dev, fp_dev->reset_gpio, "goodix_reset");
    if (rc) {
    ...
    fp_dev->irq_gpio = of_get_named_gpio(np, "goodix,gpio_irq", 0);
    if (fp_dev->irq_gpio < 0) {
        pr_err("falied to get irq gpio!\n");
        return fp_dev->irq_gpio;
    }

    rc = devm_gpio_request(dev, fp_dev->irq_gpio, "goodix_irq");
    if (rc) {
