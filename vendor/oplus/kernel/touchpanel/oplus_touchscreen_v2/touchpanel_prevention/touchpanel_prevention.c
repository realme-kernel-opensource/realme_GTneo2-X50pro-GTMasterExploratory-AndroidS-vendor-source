...
static enum large_judge_status large_shape_judged(
	struct kernel_grip_info *grip_info, uint16_t side,
	...
	if (((side >> TYPE_LONG_SIDE) & 0x01) || ((side >> TYPE_SHORT_SIDE) & 0x01)) {
		if (delta_time_ms <= grip_info->large_detect_time_ms) {
			...
			if ((side >> TYPE_LONG_SIDE) & 0x01) {
				...
				} else if (cur_p.rx_press != 0 && !(grip_info->touch_dir == VERTICAL_SCREEN && grip_info->first_point[index].y <= grip_info->max_y / 2)) {
					/* long corner side*/
					if (cur_p.rx_press >= long_side_para.large_palm_thd_1 || (cur_p.rx_er >= long_side_para.large_palm_thd_2 &&
							delta_time_ms <= grip_info->large_detect_time_ms / 5)) {
...
static void mask_potential_mistouch(struct kernel_grip_info *grip_info, struct point_info *points, int id)
{
	...
	for (index = 0; index < TOUCH_MAX_NUM; index++) {
		...
		if ((delta_time_ms < debounce_ms) && !out_range && (TYPE_REJECT_HOLD == grip_info->large_reject[index])) {
			/*already down id is under suspicious detect time*/
...
static bool large_reject_covered(struct kernel_grip_info *grip_info, struct point_info *points, int id)
{
	...
	for (index = 0; index < TOUCH_MAX_NUM; index++) {
		if (UP_POINT == grip_info->large_point_status[index]) {
			...
			/*new down id is under large up suspicious detect time*/
			under_time_interval = abs(grip_info->last_frame_point[index].time_ms - grip_info->first_point[id].time_ms) < debounce_ms;
		} else if (DOWN_POINT == grip_info->large_point_status[index]) {
			/*reject id must be different from judge id*/
			if (index == id || grip_info->points_pos[index] != pos || TYPE_REJECT_DONE != grip_info->large_reject[index]) {
				continue;
			}
...
static bool corner_exit_matched(struct kernel_grip_info *grip_info, struct point_info *points, int index)
{
	...
	if ((STATUS_CENTER_DOWN != grip_info->points_center_down[index]) && (TYPE_CORNER_LARGE_SIZE != grip_info->large_finger_status[index])
...
static bool large_exit_matched(struct kernel_grip_info *grip_info, struct point_info *points, int index)
{
	...
	if (STATUS_CENTER_DOWN != grip_info->points_center_down[index]) {
		if ((POS_LONG_LEFT == pos || POS_LONG_RIGHT == pos) && (TYPE_PALM_LONG_SIZE != grip_info->large_finger_status[index])
																	&& (TYPE_LONG_FINGER_HOLD != grip_info->large_finger_status[index])) {
...
static enum large_judge_status large_shape_judged_V2(struct kernel_grip_info *grip_info, struct point_info *points, int index)
{
	...
	uint16_t x_coupling_result = 0, y_coupling_result = 0, startx_coupling_result = 0, starty_coupling_result = 0;
	...
	if ((POS_LONG_LEFT == pos) || (POS_LONG_RIGHT == pos)) {
		...
		if (grip_info->corner_eliminate_point_support) {
			if (research_point_landed(grip_info, 0, points, index)) {
				judge_status = JUDGE_LARGE_OK;
		...
		if ((pos != POS_VERTICAL_LEFT_CORNER) && (pos != POS_VERTICAL_RIGHT_CORNER)) {
			if ((STATUS_CENTER_DOWN == grip_info->points_center_down[index]) &&
											(TYPE_REJECT_DONE == grip_info->last_large_reject[index]) && (pos == grip_info->last_points_pos[index])) {
				judge_status = JUDGE_LARGE_OK;
	...
	for (m_index = 0; m_index < TOUCH_MAX_NUM; m_index++) {
		if (((obj_attention & TOUCH_BIT_CHECK) >> m_index) & 0x01) {      /*finger down*/
			...
			grip_info->point_unmoved[m_index] = (abs(points[m_index].x - grip_info->first_point[m_index].x) <= 0
													&& abs(points[m_index].y - grip_info->first_point[m_index].y) <= 0);
			record_point_info(grip_info, TYPE_LATEST_POINT, m_index, points[m_index]);              /*record latest different points*/
			     ...
			     int obj_attention, struct point_info *points)
{
	...
	if (!grip_info->p_ts || !grip_info || !points) {
...
static const struct key_addr key_addr_arrays[] = {
	{"large_corner_exit_distance",		(int)&(((struct kernel_grip_info *)0)->large_corner_exit_distance)},
	{"large_corner_detect_time_ms",		(int)&(((struct kernel_grip_info *)0)->large_corner_detect_time_ms)},
	{"large_corner_debounce_ms",		(int)&(((struct kernel_grip_info *)0)->large_corner_debounce_ms)},
	{"large_corner_width",			(int)&(((struct kernel_grip_info *)0)->large_corner_width)},
	{"large_corner_height",			(int)&(((struct kernel_grip_info *)0)->large_corner_height)},
	{"xfsr_corner_exit_thd",		(int)&(((struct kernel_grip_info *)0)->xfsr_corner_exit_thd)},
	{"yfsr_corner_exit_thd",		(int)&(((struct kernel_grip_info *)0)->yfsr_corner_exit_thd)},
	{"exit_match_thd",			(int)&(((struct kernel_grip_info *)0)->exit_match_thd)},
	{"trx_reject_thd",			(int)&(((struct kernel_grip_info *)0)->trx_reject_thd)},
	{"rx_reject_thd",			(int)&(((struct kernel_grip_info *)0)->rx_reject_thd)},
	{"tx_reject_thd",			(int)&(((struct kernel_grip_info *)0)->tx_reject_thd)},
	{"fsr_stable_time_thd",			(int)&(((struct kernel_grip_info *)0)->fsr_stable_time_thd)},
	{"single_channel_x_len",		(int)&(((struct kernel_grip_info *)0)->single_channel_x_len)},
	{"single_channel_y_len",		(int)&(((struct kernel_grip_info *)0)->single_channel_y_len)},
	{"normal_tap_min_time_ms",		(int)&(((struct kernel_grip_info *)0)->normal_tap_min_time_ms)},
	{"normal_tap_max_time_ms",		(int)&(((struct kernel_grip_info *)0)->normal_tap_max_time_ms)},
	{"long_start_coupling_thd",		(int)&(((struct kernel_grip_info *)0)->long_start_coupling_thd)},
	{"long_stable_coupling_thd",		(int)&(((struct kernel_grip_info *)0)->long_stable_coupling_thd)},
	{"long_detect_time_ms",			(int)&(((struct kernel_grip_info *)0)->long_detect_time_ms)},
	{"long_hold_changed_thd",		(int)&(((struct kernel_grip_info *)0)->long_hold_changed_thd)},
	{"long_hold_maxfsr_gap",		(int)&(((struct kernel_grip_info *)0)->long_hold_maxfsr_gap)},
	{"long_hold_divided_factor",		(int)&(((struct kernel_grip_info *)0)->long_hold_divided_factor)},
	{"long_hold_debounce_time_ms",		(int)&(((struct kernel_grip_info *)0)->long_hold_debounce_time_ms)},
	{"xfsr_normal_exit_thd",		(int)&(((struct kernel_grip_info *)0)->xfsr_normal_exit_thd)},
	{"yfsr_normal_exit_thd",		(int)&(((struct kernel_grip_info *)0)->yfsr_normal_exit_thd)},
	{"xfsr_hold_exit_thd",			(int)&(((struct kernel_grip_info *)0)->xfsr_hold_exit_thd)},
	{"yfsr_hold_exit_thd",			(int)&(((struct kernel_grip_info *)0)->yfsr_hold_exit_thd)},
	{"large_reject_debounce_time_ms",	(int)&(((struct kernel_grip_info *)0)->large_reject_debounce_time_ms)},
	{"report_updelay_ms",			(int)&(((struct kernel_grip_info *)0)->report_updelay_ms)},
	{"short_start_coupling_thd",		(int)&(((struct kernel_grip_info *)0)->short_start_coupling_thd)},
	{"short_stable_coupling_thd",		(int)&(((struct kernel_grip_info *)0)->short_stable_coupling_thd)},
	{"short_hold_changed_thd",		(int)&(((struct kernel_grip_info *)0)->short_hold_changed_thd)},
	{"short_hold_maxfsr_gap",		(int)&(((struct kernel_grip_info *)0)->short_hold_maxfsr_gap)},
	{"large_top_width",			(int)&(((struct kernel_grip_info *)0)->large_top_width)},
	{"large_top_height",			(int)&(((struct kernel_grip_info *)0)->large_top_height)},
	{"large_top_exit_distance",		(int)&(((struct kernel_grip_info *)0)->large_top_exit_distance)},
	{"edge_swipe_narrow_witdh",		(int)&(((struct kernel_grip_info *)0)->edge_swipe_narrow_witdh)},
	{"edge_swipe_exit_distance",		(int)&(((struct kernel_grip_info *)0)->edge_swipe_exit_distance)},
	{"long_strict_start_coupling_thd",	(int)&(((struct kernel_grip_info *)0)->long_strict_start_coupling_thd)},
	{"long_strict_stable_coupling_thd",	(int)&(((struct kernel_grip_info *)0)->long_strict_stable_coupling_thd)},
	{"trx_strict_reject_thd",		(int)&(((struct kernel_grip_info *)0)->trx_strict_reject_thd)},
	{"rx_strict_reject_thd",		(int)&(((struct kernel_grip_info *)0)->rx_strict_reject_thd)},
	{"tx_strict_reject_thd",		(int)&(((struct kernel_grip_info *)0)->tx_strict_reject_thd)},
	{"short_strict_start_coupling_thd",	(int)&(((struct kernel_grip_info *)0)->short_strict_start_coupling_thd)},
	{"short_strict_stable_coupling_thd",	(int)&(((struct kernel_grip_info *)0)->short_strict_stable_coupling_thd)},
	{"xfsr_strict_exit_thd",		(int)&(((struct kernel_grip_info *)0)->xfsr_strict_exit_thd)},
	{"yfsr_strict_exit_thd",		(int)&(((struct kernel_grip_info *)0)->yfsr_strict_exit_thd)},
	{"corner_move_rejected",		(int)&(((struct kernel_grip_info *)0)->corner_move_rejected)},
};
